 ...
 * @async
 * @description Fetches all users from the database, including the number of baths they have created.
...
exports.findAllUsers = async (req, res) => {
  ...
  try {
     ...
     * Aggregates user data to include the count of baths per user and populates the 'roles' field with the role name.
     */
    const usersWithBathCount = await User.aggregate([
      {
        $lookup: {
          from: "baths",
          localField: "_id",
      ...
      .skip(skipIndex)
      .limit(limit);
    res.status(200).json(usersWithBathCount);
  } catch (error) {
